 ...
 *
 * Reconciliação de duas versões (seção 2 do design-handoff/HANDOFF.md):
 *
 * - Do repositório fica a ARQUITETURA: quem imprime a marca é o servidor, num
 *   <style> dentro do HTML (app/(app)/layout.tsx). É isso que elimina o flash de
 *   marca. `brandCss` e `brandVars` continuam sendo o contrato que ele consome.
 * - Do pacote do redesenho fica a MATEMÁTICA: contraste WCAG 2.1 real no lugar
 *   da decisão por luminância YIQ, escala de 10 tons em Oklch, e a regra do
 *   degrau aprovado mais próximo. São funções puras, sem DOM.
 * - `applyBrandTokens` sai do boot. Escrever no documentElement depois da
 *   hidratação é justamente o que causava o flash; sobrou só como
 *   pré-visualização ao vivo na tela de Marca, enquanto o admin digita.
 *
 * Durante a transição saem DOIS conjuntos de variáveis na mesma folha:
 * `--brand-*` para o sistema novo e `--primary*` para as telas ainda não
 * migradas. O bloco LEGADO do globals.css e estas cinco variáveis saem juntos,
 * no Passo 12.
 *
 * Nenhuma cor daqui vira estado, alerta ou dado: semântica não herda marca.
 */
...
export function parseHex(input: string): Rgb | null {
  const hex = String(input ?? '').trim().replace(/^#/, '').toLowerCase()
...
/** Normaliza para `#RRGGBB`, ou null quando a cor não serve. */
export function normalizeHex(color: string | null | undefined): string | null {
  const rgb = parseHex(String(color ?? ''))
 ...
 * Nunca há `#fff` fixo sobre a marca: todo consumidor lê `--brand-contrast` ou
 * `--brand-on-subtle`, definidos por esta função.
 */
...
export function generateBrandScale(primary: string): BrandScale {
  const rgb = parseHex(primary) ?? parseHex(DEFAULT_PRIMARY)!
...
export function resolveBrandTokens(primary: string): BrandTokens {
  const normalized = normalizeHex(primary) ?? DEFAULT_PRIMARY
...
/* ──────────────── variáveis CSS: o contrato do servidor ─────────────── */

/**
 * As 15 variáveis do sistema novo mais as 5 do legado, na mesma folha.
 *
 * As cinco de baixo existem porque 33 arquivos ainda leem `var(--primary)` e
 * companhia. O valor delas vem da conta nova, não da antiga: `--primary-contrast`
 * é escolhido por WCAG e `--primary-text` é o tom que passa 4,5:1 sobre o fundo
 * esmaecido, no lugar da multiplicação por 0,55 que existia antes. Saem no
 * Passo 12, junto com o bloco LEGADO do globals.css.
 */
export function brandVars(color: string | null | undefined): Record<string, string> {
  const tokens = resolveBrandTokens(color ?? DEFAULT_PRIMARY)
  const rgb = parseHex(tokens.primary)!

  const vars: Record<string, string> = {}
  for (const step of BRAND_STEPS) vars[`--brand-${step}`] = tokens.scale[step]
  vars['--brand-action'] = tokens.action
  vars['--brand-contrast'] = tokens.contrast
  vars['--brand-subtle'] = tokens.subtle
  vars['--brand-on-subtle'] = tokens.onSubtle
  vars['--brand-focus'] = tokens.focus

  vars['--primary'] = tokens.primary
  vars['--primary-dim'] = `rgba(${rgb.r},${rgb.g},${rgb.b},0.12)`
  vars['--primary-mid'] = `rgba(${rgb.r},${rgb.g},${rgb.b},0.40)`
  vars['--primary-text'] = tokens.onSubtle
  vars['--primary-contrast'] = tokens.contrast

  return vars
}

/**
 * As mesmas variáveis como folha de estilo, para o servidor mandar junto com o
 * HTML. Todo valor passa por normalização antes de virar texto, então nada além
 * de cor entra na folha — o hex gravado no banco não injeta CSS.
 */
...
/**
 * Escrita no elemento raiz — SOMENTE para pré-visualização ao vivo na tela de
 * Marca, enquanto o admin digita. No boot quem manda é a folha do servidor: usar
 * isto na carga traz de volta o flash de marca que a impressão no HTML resolveu.
 */
export function applyBrandTokens(
  ...
  for (const [nome, valor] of Object.entries(brandVars(primary))) {
